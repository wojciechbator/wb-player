...
class GenericNode extends Component {
    ...
    componentDidMount() {
        if (this.props.isMaster)
            ...
            this.props.node.type ? this.setState({
                    value: this.props.node.gain ? this.props.node.gain.value : 50,
                    name: this.props.node.type
                }) :
                this.setState({
                    value: this.props.node.gain ? this.props.node.gain.value : 0.5,
                    name: this.props.node.constructor.name
                });
            this.props.node.constructor.name === 'WaveShaperNode' || this.props.node.constructor.name === 'DelayNode' ?
                this.setState({
                    isWaveShaperOrDelay: true,
                    name: this.props.node.constructor.name
                }) : this.setState({isWaveShaperOrDelay: false, name: this.props.node.constructor.name});
        }
    }
            ...
            <div>
                <Fieldset className='node-body' legend={this.state.name} toggleable={true}>
                    <div className="generic-node-wrapper">
                        {this.state.isWaveShaperOrDelay ? <h3 className='node-inner-label'>{this.state.name}</h3> :
                            <h3>value: {this.state.type ? this.state.value : Math.round(this.state.value * 100)}</h3>}
                        {!this.state.isWaveShaperOrDelay &&
                        <Slider className='node-slider' orientation='vertical' animate={true}
                                value={this.state.type ? this.state.value : Math.round(this.state.value * 100)}
                                onChange={this.onValueChange}/>}
                        <Button className='remove-button' label='Remove' onClick={this.removeNode}
